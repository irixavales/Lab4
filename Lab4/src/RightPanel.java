...
public class RightPanel extends JPanel {
	...
	public void transitionDiagonal45LR(Graphics g, BufferedImage leftImage) {
		...
		if (width > height) {
			...
			// Image with larger height than  width	
	...
	public void transitionRandom (Graphics g, BufferedImage leftImage) {
		...
		for (int i=0; i<=width*height; i++) {
			...
			int xRand = rand.nextInt(width);
			int yRand = rand.nextInt(height);
			int pixelColor = leftImage.getRGB(xRand, yRand);
			img.setRGB(xRand, yRand, pixelColor);
			repaint();
